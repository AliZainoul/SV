...
using namespace std;
int g = 10;
double fi(double x, double y) { return abs(x/y) + sqrt(g*y); };
...
Vector::Vector(Vector const & a, Vector const & b)
{
	if (a.size() != b.size()) error("Bad vector sizes; it should match!");
	M_size = a.size();
	for(int k=0; k<a.size(); k++) M_data[k] = fi(a[k],b[k]);
}
